...
function Testdata() {
  ...
  return (
    <div className=' w-full h-auto flex flex-col items-center'>
      ...
      <form onSubmit={handleSubmit} className=' mt-5 gap-3 w-1/2 flex flex-row mb-5 justify-center items-center'>
        <input type="text" name='username' placeholder="Enter a GitHub username" value={searchTerm} onChange={handleChange} className=' w-full py-3
         rounded-2xl text-center bg-[#E8F0FE] text-zinc-800 px-2 outline-none' />
        <button type="submit" className=' hover:bg-blue-600 py-2 bg-blue-500 rounded-xl ml-2 w-1/4'>Search</button>
      </form>
      {error && <p>{error}</p>}
